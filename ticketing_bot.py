...
while not ticketing_available:
    try:
        ...
        try:
            if not pre_sale_identify:
                pre_sale_identify_btn = driver.find_element_by_xpath('//*[@id="sportsLeisureDetail"]/div[2]/div[4]/div[4]/div[2]/div[5]/a[2]')
                btn_click(pre_sale_identify_btn)
                switch_to_popup_window(driver)

                name_input = driver.find_element_by_id("AuthField1")
                name_input.send_keys(os.environ['USER_NAME'])

                membership_input = driver.find_element_by_id("AuthField2")
                membership_input.send_keys(os.environ['MEMBERSHIP_CODE'])

                accept_btn = driver.find_element_by_xpath('//*[@id="logpop"]/div[2]/div[2]/a')
                accept_btn.click()
...
while not selected:
    for seat in seats:
        try:
            if any(section_name in seat.get_attribute('title') for section_name in section_names):
